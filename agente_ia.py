...
# -------------------------------------------
# Treinamento com a visualização do mesmo
# -------------------------------------------
pygame.init()
...
# -------------------------------------------
# Executando o agente treinado anteriormente
# -------------------------------------------
pygame.display.init()
screen = pygame.display.set_mode((WIDTH, HEIGHT))
...
while running:
    ...
    # Desenha o agente atual
    if not reached_goal:
        draw_agent(screen, agent_pos, BLUE)

    pygame.display.flip()
    clock.tick(30)  # FPS alto para manter janela fluida, senão parecia sequÊncia de fotografia
    ...
    if not reached_goal:
        if agent_pos != GOAL:
            ...
            if next_pos == agent_pos:
                ...
                agent_pos = next_pos
                time.sleep(0.8)  # <-- controle de velocidade da execução, estava dando BO d+
        else:
            reached_goal = True
